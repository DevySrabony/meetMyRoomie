...
const Navbar = () => {
  ...
  return (
    <nav className="bg-orange-50 dark:bg-gray-900 shadow-md px-4 md:px-24 py-5 sticky top-0 z-50 nav-font">
      <div className="flex justify-between items-center">
        {/* Logo */}
        <Link to="/" className="text-2xl font-bold text-orange-500">
          MeetMyRoomie
        ...
        <div className="flex items-center gap-4">
          {!user ? (
            <>
              <Link
                to="/login"
                className="px-4 py-1 text-sm border border-orange-400 text-orange-500 rounded hover:bg-orange-100 transition"
              >
                Login
              </Link>
              <Link
      ...
      {/* Mobile Nav */}
      <div className="md:hidden mt-3 space-y-2 space-x-3 text-sm font-medium text-gray-700">
        {navLinks}
      </div>
    </nav>
  );
